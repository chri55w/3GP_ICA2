...
void MyController::windowControlWillStart(std::shared_ptr<tygra::Window> window) {
...
void MyController::windowControlDidStop(std::shared_ptr<tygra::Window> window) {
	window->setView(nullptr);
}

void MyController::windowControlViewWillRender(std::shared_ptr<tygra::Window> window) {
	scene_->update();
...
void MyController::windowControlMouseMoved(std::shared_ptr<tygra::Window> window, int x, int y) {
	static int prev_x = x;
	static int prev_y = y;
	if (camera_turn_mode_) {
		int dx = x - prev_x;
		...
		scene_->getCamera().setRotationalVelocity(glm::vec2(-dx * mouse_speed, -dy * mouse_speed));
	}
	prev_x = x;
	prev_y = y;
}

void MyController::windowControlMouseButtonChanged(std::shared_ptr<tygra::Window> window, int button_index, bool down) {
...
void MyController::windowControlMouseWheelMoved(std::shared_ptr<tygra::Window> window, int position) {

}

void MyController::windowControlKeyboardChanged(std::shared_ptr<tygra::Window> window, int key_index, bool down) {
	switch (key_index) {
...
void MyController::windowControlGamepadAxisMoved(std::shared_ptr<tygra::Window> window, int gamepad_index, int axis_index, float pos) {
	...
	switch (axis_index) {
		...
		case tygra::kWindowGamepadAxisRightThumbX:
			...
			scene_->getCamera().setRotationalVelocity(glm::vec2(camera_rotate_speed_[0] * rotate_speed, camera_rotate_speed_[1] * rotate_speed));
			break;
...
void MyController::windowControlGamepadButtonChanged(std::shared_ptr<tygra::Window> window, int gamepad_index, int button_index, bool down) {
...
void MyController::updateCameraTranslation() {
	...
	scene_->getCamera().setLinearVelocity(glm::vec3(sideward_speed, 0, forward_speed));
}
